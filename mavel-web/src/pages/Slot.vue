<template>
  ...
  <div class="header-label">
    <h3 class="text-h3 text-weight-bold">{{ $t('main.title.slots') }}</h3>
      ...
      <q-intersection
        ...
        <q-card class="q-ma-sm">
          ...
          <!-- <q-img
            ...
            class="link"
            :class="{
              'img-games': !nowLoading.includes(String(item.gameCode)),
            }"
            @click="clickGame(item)">
            <div class="carousel-bg" v-if="nowLoading.includes(String(item.gameCode))">
              <q-spinner size="5rem" />
            </div>
          </q-img> -->
          <q-card-section>
...
// import HotGameList from 'src/components/HotGameList.vue';
...
// const { nowLoading } = storeToRefs(gamesStore.play());
