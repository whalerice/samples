<template>
  ...
  <div class="text-right q-mb-sm">
    <ExcelDownload :data="searchSendData" url="Manager/CalculateHistory" />
  </div>
  <q-table
    flat
    ref="tableRef"
...
import { FilterDefault, RewardType, SiteGrade, UserCalculateState } from 'src/components/models/enums';
...
const onRequest = async (props: any) => {
  ...
  searchSendData.value.rewardType = 1;
  ...
  try {
    const response = await apis.manager.CalculateHistory(send);
...
const onStatus = async (data: any, state: boolean) => {
  ...
  try {
    const response = await apis.manager.UserCalculateSetting(send);
    if (response.data.errCode === 0) {
      onRefresh();
    }
    loading.value = false;
