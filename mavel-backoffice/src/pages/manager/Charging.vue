    ...
    <template v-slot:body-cell-state="props">
      <q-td :props="props">
        <template
          ...
          ">
          <a-space :size="5">
            <a-button type="primary" @click="onChangState(props.row, DefaultState.Approval)" ghost size="small">
              {{ $t('common.button.approval') }}
            </a-button>
            <a-button type="primary" danger @click="onChangState(props.row, DefaultState.Refusal)" ghost size="small">
...
const columns = ref<any[]>([
  ...
  },
  {
    ...
    field: 'order_Name',
    required: true,
    label: () => t('table.thead.order_name'),
    align: 'left',
    sortable: true,
  },
  {
    ...
    field: 'currency_Type',
    required: true,
    label: () => t('table.thead.currency_type'),
    align: 'left',
    sortable: true,
  },
  {
  ...
  },
  {
    ...
    required: true,
    label: () => t('table.thead.registration_date'),
    align: 'left',
    sortable: true,
  },
  {
    ...
    required: true,
    label: () => t('table.thead.processing_date'),
    align: 'left',
    sortable: true,
  },
]);
...
const onRequest = async (props: any) => {
  ...
  try {
    const response = await apis.manager.ChargeList(send);
...
const onChangState = async (data: any, state: number) => {
  ...
  try {
    const response = await apis.manager.ChargeApproval(send);
    if (response.data.errCode === 0) {
      onRefresh();
    }
  } catch (error) {
