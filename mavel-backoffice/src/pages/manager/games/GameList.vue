<template>
  <a-space class="q-mb-md">
    <a-button type="primary" @click="addNewGame()" :loading="addingGames">
      <template #icon>
        <plus-outlined />
      </template>
      {{ $t('common.button.add_new_game') }}
    </a-button>
  </a-space>
  ...
  <q-table
    ...
    <template v-slot:body-cell-game_designation="props">
      <q-td :props="props">
        <a-space :size="5">
          <q-btn
            outline
            ...
            color="red"
            class="ant-btn-sm"
            v-if="props.row.isHot === 0"
            @click="onDesignate(props.row, GameDesignation.Hot)" />
          <q-btn
            outline
            no-caps
            label="Favorite"
            color="indigo"
            class="ant-btn-sm"
            v-if="props.row.isHit === 0"
            @click="onDesignate(props.row, GameDesignation.Favorite)" />
        </a-space>
      </q-td>
    </template>
  </q-table>
...
const search = ref<ISearch>({ type: FilterDefault.All, category: GameCategory.All, date: null, text: '' });
const searchSendData = ref<ISearch>({ type: FilterDefault.All, category: GameCategory.All, date: null, text: '' });
...
const columns = ref<any[]>([
  ...
  {
    name: 'game_designation',
    field: 'game_designation',
    required: true,
    label: 'table.thead.game_designation',
    align: 'center',
    sortable: true,
  },
  {
...
const onRequest = async (props: any) => {
  ...
  try {
    const response = await apis.manager.GameList(send);
...
const onChange = async (e: any) => {
  try {
    const response = await apis.manager.GameState({
      idx: e.idx,
      state: e.state,
    });
    if (response.data.errCode === 0) {
...
const onDesignate = async (e: any, target: number) => {
  try {
    const response = await apis.manager.GameHotHitModify({
      game_Idx: e.idx,
      solution_Idx: e.solution_Idx,
      category: target,
    });
    if (response.data.errCode === 0) {
      message.success(response.data.errMessage);
...
const addNewGame = async () => {
  ...
  try {
    const response = await apis.manager.GameUpdate();
    if (response.data.errCode === 0) {
