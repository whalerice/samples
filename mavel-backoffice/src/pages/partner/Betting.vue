...
const resultType = ref<IResultType[]>([
  { value: true, label: () => t('common.text.win') },
  { value: false, label: () => t('common.text.lose') },
]);
...
const onRequest = async (t: any) => {
  ...
  try {
    const response = await apis.partner.BettingHistory(send);
...
onMounted(() => {
  let useOption = [FilterDefault.All, FilterDefault.IDNick];
